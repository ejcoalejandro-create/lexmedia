...
// ─────────────────────────────────────────────
// CREDENCIALES — completar antes de entregar
// ─────────────────────────────────────────────
const CONFIG = {
...
const REDES = [
  { id: "linkedin",  label: "LinkedIn",  mono: "#111" },
  { id: "instagram", label: "Instagram", mono: "#111" },
  { id: "twitter",   label: "X / Twitter", mono: "#111" },
  { id: "facebook",  label: "Facebook",  mono: "#111" },
  { id: "threads",   label: "Threads",   mono: "#111" },
];
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          ...
          { type: "text", text: `Analiza esta imagen de la competencia. Devuelve SOLO JSON:
{
  "estilo": "descripción del estilo visual",
  "tema": "tema en 3-5 palabras",
  "elementos": "elementos visuales clave",
  "tono": "tono emocional",
  "prompt_inspirado": "prompt en inglés para SDXL, imagen distinta pero mismo tema, formato cuadrado, profesional para redes sociales",
...
// ─────────────────────────────────────────────
const POST_PROMPTS = {
  linkedin:  `Post profesional LinkedIn 600-1100 chars. JSON: {"post":"..."}. Hook + párrafos cortos + 3-5 hashtags. Tono profesional jurídico.`,
  instagram: `Caption Instagram 700-1500 chars. JSON: {"post":"..."}. Emojis moderados + 5-8 hashtags + CTA. Tono cercano pero serio.`,
...
async function procesarURL(url, redes, onLog) {
  ...
  const r = await callClaude(
    `Experto en content marketing jurídico. Devuelve SOLO JSON: {"resumen":"150-200 palabras en español, tono profesional","tema":"3 palabras"}`,
    `URL: ${url}\n\n${contenido}`
  );
  const resumen = r.resumen || r.raw || "Contenido procesado.";
  const tema = r.tema || "contenido jurídico";
  log(`Resumen generado sobre: ${tema}`);
  const posts = {};
  ...
  log("Generando prompt visual…");
  const ip = await callClaude(
    `Prompt en inglés para imagen profesional redes sociales para despacho de abogados. JSON: {"prompt":"...SDXL..."}`,
    `Tema: ${tema}\nResumen: ${resumen}`
  );
  const imagePrompt = ip.prompt || `Professional legal office illustration, clean minimalist design, black and white tones`;
  let imageUrl = null;
  if (CONFIG.FAL_API_KEY) {
    log("Generando imagen…");
    try { imageUrl = await generateImageFal(imagePrompt); log("Imagen generada correctamente."); }
    catch(e) { log(`Advertencia: ${e.message}`); }
  } else {
    imageUrl = `https://placehold.co/800x800/f8f8f8/222222?text=${encodeURIComponent(tema)}`;
  }
...
async function procesarImagenCompetencia(file, redes, onLog) {
  ...
  for (const red of redes) {
    ...
    const p = await callClaude(
      ...
      `Tema: ${analisis.tema}\nDescripción: ${contexto}\nEstilo competencia: ${analisis.estilo}\nContexto: estudio jurídico profesional`
    );
  ...
  if (CONFIG.FAL_API_KEY) {
    ...
    imageUrl = `https://placehold.co/800x800/f8f8f8/222222?text=${encodeURIComponent(analisis.tema || "Tu versión")}`;
  }
...
const S = {
  ...
  logoMark: {
    ...
    fontSize: 14, color: "#fff", fontStyle: "italic", fontFamily: "Georgia, serif",
    letterSpacing: 1,
  },
  logoText: {
  ...
  logoSub: {
    ...
    fontFamily: "'Arial', sans-serif", marginTop: 1,
  },
  headerRight: {
  ...
  pill: {
    ...
    color: "#888", fontFamily: "Arial, sans-serif",
    border: "0.5px solid #444", padding: "3px 10px",
  },
  main: {
    maxWidth: 860,
    margin: "0 auto",
  ...
  sectionTitle: {
    ...
    color: "#888", fontFamily: "Arial, sans-serif", marginBottom: 20,
  },
  // Modo toggle
  modeRow: {
  ...
  log: {
    ...
    padding: "16px 20px",
    fontFamily: "Arial, sans-serif",
    fontSize: 11, color: "#666", lineHeight: 2,
  ...
  postCardDot: {
    width: 8, height: 8, background: "#111", borderRadius: "50%",
  },
  postCardLabel: {
    fontSize: 10, letterSpacing: 2, textTransform: "uppercase",
    fontFamily: "Arial, sans-serif", fontWeight: 600, color: "#333",
    flex: 1,
  },
  postCardChars: {
    fontSize: 10, color: "#bbb", fontFamily: "Arial, sans-serif",
  },
  copyBtn: (copied) => ({
  ...
  const icons = {
    ...
    threads: (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
        ...
        <path d="M9 9.5c0-1.5 1.2-2.5 3-2.5s3 1 3 2.5-1.5 2-3.5 2.5c-2 .5-3 1.5-3 3s1.5 2.5 3.5 2.5 3.5-1 3.5-2.5" />
      </svg>
    ),
  };
...
function PostCard({ red, post, imageUrl }) {
  ...
  return (
    <div style={S.postCard}>
      <div style={S.postCardHeader(red.id)}>
      ...
      {imageUrl && (
        <img src={imageUrl} alt="" style={{ width: "100%", maxHeight: 220, objectFit: "cover", display: "block", borderBottom: "1px solid #ebebeb" }} />
      )}
...
function DropZone({ file, setFile, preview, setPreview }) {
  ...
  const onFile = f => {
    ...
    setFile(f); const r = new FileReader(); r.onload = e => setPreview(e.target.result); r.readAsDataURL(f);
  };
  return (
    <div
      ...
      style={{
        border: `1px dashed ${file ? "#111" : "#ccc"}`,
        padding: file ? 0 : "40px 20px",
      ...
      {file ? (
        <>
          ...
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, background: "rgba(0,0,0,.6)", padding: "8px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ color: "#fff", fontSize: 11, fontFamily: "Arial, sans-serif", letterSpacing: 1 }}>{file.name.slice(0, 40)}</span>
            <button onClick={e => { e.stopPropagation(); setFile(null); setPreview(null); }} style={{ background: "none", border: "1px solid #fff", color: "#fff", padding: "2px 8px", cursor: "pointer", fontSize: 11, fontFamily: "Arial, sans-serif" }}>Quitar</button>
          </div>
        </>
      ) : (
        <>
...
export default function App() {
  ...
  const reset = () => { setResult(null); setLogs([]); setError(null); setStep(0); };

  const run = async () => {
    setProcessing(true); setError(null); setResult(null); setLogs([]); setStep(1);
    try {
      const res = modo === "url"
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e0e0e0", padding: "20px 48px", display: "flex", alignItems: "baseline", gap: 24 }}>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 400, fontFamily: "Georgia, serif", color: "#111", letterSpacing: -0.5 }}>
          Publicador Inteligente
        </h1>
        <span style={{ fontSize: 12, color: "#aaa", fontFamily: "Arial, sans-serif", letterSpacing: 1 }}>
          URL o imagen → resumen → imagen → publicación en redes
        </span>
      </div>
      ...
      <main style={S.main}>
                ...
                <div key={e.id} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  ...
                  {entries.length > 1 && (
                    <button onClick={() => setEntries(p => p.filter(x => x.id !== e.id))}
                      style={{ background: "#fff", border: "1px solid #ddd", color: "#aaa", padding: "0 16px", cursor: "pointer", fontFamily: "Arial", fontSize: 16 }}>✕</button>
                  )}
                </div>
              ))}
        ...
        {result && (
          <div>
            {/* Comparativa en modo competencia */}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, border: "1px solid #e0e0e0" }}>
                  ...
                  <div>
                    ...
                    {result.imageUrl
                      ...
                      : <div style={{ aspectRatio: "1", background: "#f8f8f8", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#ccc", fontFamily: "Arial" }}>Configura fal.ai para imagen real</div>
                    }
                  </div>
                </div>
              </div>
            ...
            {!result.modoCompetencia && result.imageUrl && (
              <div style={S.section}>
                ...
                <div style={{ padding: "8px 0", fontSize: 10, color: "#bbb", fontFamily: "Arial, sans-serif", letterSpacing: 1 }}>
                  Prompt: {result.imagePrompt?.slice(0, 100)}…
                </div>
              </div>
            )}
            ...
            <div style={S.section}>
              ...
              <div>
                {redes.map(redId => {
                  ...
                  return result.posts[redId]
                    ? <PostCard key={redId} red={red} post={result.posts[redId]} imageUrl={result.imageUrl} />
                    : null;
                })}
              </div>
            </div>
